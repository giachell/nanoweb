/*!
 * NanoWeb JavaScript Library v1.0.0
...
$(document).ready(function load()
{
 ...
 /* HELP commands */
 $('#controlsHelp').click(function () {
     $('#controlsHelp-overlay').show();
 });

    $('#controlsHelp-overlay-close').click(function () {
        //console.log('#controlsHelp-overlay-close clicked');
        $('#controlsHelp-overlay').hide();
    });

    $('#controlsHelp-overlay').click(function () {
        //console.log('#controlsHelp-overlay-close clicked');
        $('#controlsHelp-overlay').hide();
    });
 ...
 /* FUNCTIONS */
    ...
    function closestPoint(pathNode, point) {
        ...
        for (var scan, scanLength = 0, scanDistance; scanLength <= pathLength; scanLength += precision) {
            if ((scanDistance = distance2(scan = pathNode.getPointAtLength(scanLength), point)) < bestDistance) {
        ...
        while (precision > 0.5) {
            var before,
                ...
                beforeDistance,
                afterDistance;
            if ((beforeLength = bestLength - precision) >= 0 && (beforeDistance = distance2(before = pathNode.getPointAtLength(beforeLength), point)) < bestDistance) {
                best = before, bestLength = beforeLength, bestDistance = beforeDistance;
            } else if ((afterLength = bestLength + precision) <= pathLength && (afterDistance = distance2(after = pathNode.getPointAtLength(afterLength), point)) < bestDistance) {
        ...
        function distance2(p, point) {
            ...
            let transform_translate_sec = $('.svgElementsContainer').attr('transform');

            let translate_values_sec = null;
            let translate_x_sec = 0.0;
            let translate_y_sec = 0.0;

            let scale_text = "";
            let scale = 1.0;


            if(transform_translate_sec!= undefined && transform_translate_sec.includes("scale"))
            {

                scale_text = transform_translate_sec.substr(transform_translate_sec.indexOf("scale("));
                scale_text= scale_text.replace("scale(","").replace(")","");
                scale = parseFloat(scale_text);
                console.log("scale svgElementsContainer: "+scale);
            }

            if(transform_translate_sec != undefined)
            {

                transform_translate_sec = transform_translate_sec.replace("translate(","");
                transform_translate_sec = transform_translate_sec.replace(")","");
                translate_values_sec = transform_translate_sec.split(",");
                translate_x_sec = parseFloat(translate_values_sec[0])/scale;
                translate_y_sec = parseFloat(translate_values_sec[1])/scale;
            }



            console.log('transform translate_x_sec, translate_y_sec: '+translate_x_sec+", "+translate_y_sec);
            console.log("ep_x: "+(p.x+parseFloat(translate_x)+translate_x_sec)+ " click point x: "+point[0]);
            console.log("ep_y: "+(p.y+parseFloat(translate_y)+translate_y_sec)+ " click point y: "+point[1]);
            var dx = (p.x+parseFloat(translate_x)+translate_x_sec) - point[0],
                dy = (p.y+parseFloat(translate_y)+translate_y_sec) - point[1];
            return dx * dx + dy * dy;
        }
    }

    function detectEdgeClick(click_x, click_y) {
        //console.log( d3.event.pageX, d3.event.pageY ); // log the mouse x,y position
        console.log( click_x, click_y ); // log the mouse x,y position
        ...
        if(lines_number > 0)
        {
            ...
            if (min_distance < 5) {
                ...
                console.log("min_distance: (<5) "+min_distance);
        ...
        await $.ajax({
            ...
            .done(
                function( data )
                {
                    ...
                    // TODO: adjust edge click
                    //svg.on('click', function (){let p_clicked = d3.mouse(this); let x_click = p_clicked[0]; let y_click = p_clicked[1]; detectEdgeClick(x_click, y_click);});
...
/*
    ...
    */
    ...
    if(expanded_nodes_list.includes(d.id))
        {
            ...
            setTimeout(function(){
                ...
                $('.svgElementsContainer').attr('transform',transform_translate_sec);
    ...
    let transform_translate_sec = $('.svgElementsContainer').attr('transform');
        ...
        svgElementsContainer = svg.append("g").attr("class","svgElementsContainer");
        ...
        svg.call(d3.zoom().on("zoom", function () {
           ...
           /*.on("click",function (e) {
            ...
            let pos_click = d3.mouse(this);
            console.log('d3 pos click (x): '+pos_click[0]);
            console.log('d3 pos click (y): '+pos_click[1]);
            console.dir(pos_click);


            detectEdgeClick(pos_click[0], pos_click[1]);
